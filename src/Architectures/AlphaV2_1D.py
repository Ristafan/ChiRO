...
class AlphaV2_1D(nn.Module):
    def __init__(self, dropout_rate=0.3, batch_norm=True, global_pooling="avg"):
        super(AlphaV2_1D, self).__init__()

        # Convolutional layers
        self.conv1_time = nn.Conv2d(1, 1, kernel_size=(3, 1), stride=1, padding=(1, 0), groups=1)
        self.conv1_freq = nn.Conv2d(1, 32, kernel_size=(1, 3), stride=1, padding=(0, 1))
        self.pool = nn.MaxPool2d(2, 2)
        self.batchnorm1 = nn.BatchNorm2d(32) if batch_norm else None

        self.conv2_time = nn.Conv2d(32, 32, kernel_size=(3, 1), stride=1, padding=(1, 0), groups=32)
        self.conv2_freq = nn.Conv2d(32, 64, kernel_size=(1, 3), stride=1, padding=(0, 1))
        self.batchnorm2 = nn.BatchNorm2d(64) if batch_norm else None

        self.conv3_time = nn.Conv2d(64, 64, kernel_size=(3, 1), stride=1, padding=(1, 0), groups=64)
        self.conv3_freq = nn.Conv2d(64, 128, kernel_size=(1, 3), stride=1, padding=(0, 1))
